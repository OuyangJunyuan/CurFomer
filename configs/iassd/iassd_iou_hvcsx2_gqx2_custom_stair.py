...
from ..base.datasets.custom_stairs import *
...
DATASET.DATA_PROCESSOR = [
    ...
    dict(NAME='sample_points',
         NUM_POINTS={'train': 8192, 'test': 8192})
]
...
MODEL = dict(
    ...
    BACKBONE_3D=dict(
        ...
        ENCODER=
        [
            dict(samplers=[dict(name='hvcs', sample=2048, voxel=[0.4, 0.4, 0.35])],
            ...
            dict(samplers=[dict(name='hvcs', sample=512, voxel=[2.0, 2.0, 1.75])],
            ...
            dict(samplers=[dict(name='ctr', sample=256, mlps=[128], class_names=DATASET.CLASS_NAMES,
    ...
    POINT_HEAD=dict(
        ...
        VOTE_SAMPLER=dict(name='ctr', sample=128, mlps=[256], class_names=DATASET.CLASS_NAMES,
                          train=dict(target={'extra_width': [0.5, 0.5, 0.5]},
        ...
        BOX_CODER=dict(name='PointBinResidualCoder', angle_bin_num=12,
                       use_mean_size=True, mean_size=[[8.5, 3.0, 2.4]]),
        TARGET_CONFIG={'method': 'mask', 'gt_central_radius': False, 'extra_width': [0.5, 0.5, 0.5]},
