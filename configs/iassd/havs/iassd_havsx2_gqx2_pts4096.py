...
DATASET.DATA_PROCESSOR = [
    ...
    dict(NAME='sample_points',
         NUM_POINTS={'train': 16384 // stride, 'test': 16384 // stride})
]
MODEL = dict(
    NAME='PointVote',
    ...
    BACKBONE_3D=dict(
        ...
        ENCODER=
        [
            dict(samplers=[dict(name='havs', sample=4096 // stride, voxel=[0.4, 0.4, 0.35], return_hash=True)],
                 groupers=[dict(name='ball', query=dict(radius=0.2, neighbour=16), mlps=[16, 16, 32], use_hash=True),
                           dict(name='ball', query=dict(radius=0.8, neighbour=32), mlps=[32, 32, 64], use_hash=True)],
                 aggregation=dict(name='cat-mlps', mlps=[64])),
            dict(samplers=[dict(name='havs', sample=1024 // stride, voxel=[2.0, 2.0, 1.75], return_hash=True)],
                 groupers=[dict(name='ball', query=dict(radius=0.8, neighbour=16), mlps=[64, 64, 128], use_hash=True),
                           dict(name='ball', query=dict(radius=1.6, neighbour=32), mlps=[64, 96, 128], use_hash=True)],
                 aggregation=dict(name='cat-mlps', mlps=[128])),
            dict(samplers=[dict(name='ctr', sample=512 // stride, mlps=[128], class_names=DATASET.CLASS_NAMES,
    ...
    POINT_HEAD=dict(
        ...
        VOTE_SAMPLER=dict(name='ctr', sample=256 // stride, mlps=[256], class_names=DATASET.CLASS_NAMES,
                          train=dict(target={'extra_width': [0.5, 0.5, 0.5]},
