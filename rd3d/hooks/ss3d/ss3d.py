...
class UnlabeledInstanceMiningModule:
    def __init__(self, cfg, ins_bank: InstanceBank):
        ...
        self.logger = create_logger(name="ins", file=self.root_path / "log.txt", scene=False)
    ...
    def missing_annotated_mining(self, dataloader):
        ...
        pbar_kwargs = dict(desc='instance mining', leave=False, disable=not dist.state.is_main_process)
        for fid in tqdm(iterable=self.raw_scene_preds_dict, **pbar_kwargs):
            ...
            match_2d_raw_pred, unmatch_2d_raw_pred = filter_out_data(raw_preds, iou_2d_keep)
            match_2d_aug_pred, unmatch_2d_aug_pred = filter_out_data(aug_preds, iou_2d_keep)
            self.score_guided_suppression(
            ...
            raw_preds = merge_dicts(match_2d_raw_pred, unmatch_2d_raw_pred)
            aug_preds = merge_dicts(match_2d_aug_pred, unmatch_2d_aug_pred)
    ...
    def predict_aug_and_raw_scenes(self, dataloader, model):
        ...
        pbar_kwargs = dict(desc='raw scenes', leave=False, disable=not dist.state.is_main_process)
        with replace_attr(dataloader.dataset.data_augmentor, data_augmentor_queue=[]):
            for batch_dict in tqdm(iterable=dataloader, **pbar_kwargs):
                ...
                self.raw_scene_preds_dict.update({
                    ...
                    zip(batch_dict['frame_id'], load_data_to_numpy(pred_dicts))
                })

        pbar_kwargs = dict(desc='aug scenes', leave=False, disable=not dist.state.is_main_process)
        ...
        self.raw_scene_preds_dict = dist.all_gather(self.raw_scene_preds_dict)
        self.aug_scene_preds_dict = dist.all_gather(self.aug_scene_preds_dict)

        if dist.state.is_main_process:
            # validation checking
            num_samples = len(dataloader) * dataloader.batch_sampler.batch_size * dist.state.num_processes
            ...
            raw_nums = np.array([len(pred[0]['pred_boxes']) for pred in self.raw_scene_preds_dict.values()])
            aug_nums = np.array([len(pred[0]['pred_boxes']) for pred in self.aug_scene_preds_dict.values()])
    ...
    def mine_unlabeled_instance_one_epoch(self, run, model, dataloader):
        self.aug_scene_preds_dict = {}
        self.raw_scene_preds_dict = {}

        if not (self.cfg.cache and self.load()):  # read from cache
            self.predict_aug_and_raw_scenes(dataloader, model)
            self.save()

        self.missing_annotated_mining(dataloader)  # perform instance mining (on main process)
...
class ReliableBackGroundMiningModule:
    def __init__(self, cfg, ins_bank: InstanceBank):
        ...
        self.root_path.mkdir(parents=True, exist_ok=True)
        self.logger = create_logger(name="bg", file=self.root_path / "log.txt", scene=False)
    ...
    def weaken(self, raw_post_processing_cfg):
        ...
        weak_post_processing.NMS_CONFIG.NMS_THRESH = 1.1
    ...
    def predict_without_nms_and_low_score_threshold(self, run, dataloader, model):
        ...
        model.eval()
        model_cfg = run.unwrap_model.model_cfg
        with replace_attr(model_cfg, POST_PROCESSING=self.weaken(model_cfg.POST_PROCESSING)):
            with replace_attr(dataloader.dataset.data_augmentor, data_augmentor_queue=[]):
                pbar_kwargs = dict(desc='background mining', leave=False, disable=not dist.state.is_main_process)
                for batch_dict in tqdm(iterable=dataloader, **pbar_kwargs):
                    dataloader.dataset.load_data_to_gpu(batch_dict)
        ...
        self.redundant_predicted_boxes = dist.all_gather(self.redundant_predicted_boxes)
        if dist.state.is_main_process:
            num_samples = len(dataloader) * dataloader.batch_sampler.batch_size * dist.state.num_processes
    ...
    def mine_reliable_background_one_epoch(self, run, dataloader, model):
        # predict reliable background or load from cache
...
class SS3DHookImpl:
    def __init__(self, cfg):
        self.cfg = cfg
        self.begin_epoch, self.end_epoch = 0, 0

        self.ins_bank = InstanceBank(
            self.cfg.instance_bank, self.cfg.root_dir, self.cfg.class_names
        )
        self.ins_miner = UnlabeledInstanceMiningModule(
            self.cfg.unlabeled_instance_mining, self.ins_bank
        )
        self.bg_miner = ReliableBackGroundMiningModule(
            self.cfg.reliable_background_mining, self.ins_bank
        )
        self.instance_filling_aug = InstanceFilling(
            **self.cfg.instance_filling, instance_bank=self.ins_bank, background_miner=self.bg_miner
        )
        self.augmentor_replacement = None

        self.assign_state_handler_to_runner()

    def assign_state_handler_to_runner(self):
        @Hook.begin_and_end
        def instance_mining_one_epoch(run, *args, **kwargs):
            self.instance_mining_one_epoch(run, *args, **kwargs)

        @Hook.begin_and_end
        def ss3d_iteratively_train_one_epoch(run, *args, **kwargs):
            self.ss3d_iteratively_train_one_epoch(run, *args, **kwargs)

        from ...core import DistributedRunner
        DistributedRunner.instance_mining_one_epoch = instance_mining_one_epoch
        DistributedRunner.ss3d_iteratively_train_one_epoch = ss3d_iteratively_train_one_epoch

    def get_ss3d_scheduler(self, run, dataloader):
        from ...core.optimization import build_scheduler

        train_steps = len(dataloader) * dist.state.num_processes * self.cfg.epochs
        new_scheduler = build_scheduler(
            self.cfg.lr, optimizer=run.optimizer.optimizer, total_steps=train_steps
        )
        return run.accelerator.prepare(new_scheduler)

    def ss3d_augmentor_queue(self, raw_augmentor):
        new_augmentor_queue = [self.instance_filling_aug]
        for ag in raw_augmentor.data_augmentor_queue:
            if type(ag).__name__ == 'DataBaseSampler':
                new_augmentor_queue.append(ag)
                break

        return replace_attr(raw_augmentor, data_augmentor_queue=new_augmentor_queue).__enter__()


@HOOKS.register()
class SS3DHook(SS3DHookImpl):
    priority = 40
    enable = True

    def before_run(self, run):
        self.begin_epoch = run.cfg.max_epochs
        run.cfg.max_epochs += self.cfg.iter_num * self.cfg.epochs
        self.end_epoch = run.cfg.max_epochs

    def instance_mining_one_epoch(self, run, model, dataloader):
        # instance mining
        self.ins_miner.mine_unlabeled_instance_one_epoch(run, model, dataloader)

        # reset scheduler for iteratively training
        run.scheduler = self.get_ss3d_scheduler(run, dataloader)

    def before_train_one_epoch(self, run, model, dataloader):
        if self.end_epoch > run.cur_epochs >= self.begin_epoch:  # [80,140)
            # background mining
            self.bg_miner.mine_reliable_background_one_epoch(run, dataloader, model)
            # training one epoch with instance filling augmentor
            self.augmentor_replacement = self.ss3d_augmentor_queue(dataloader.dataset.data_augmentor)

    def after_train_one_epoch(self, run, *args, **kwargs):
        if self.end_epoch >= run.cur_epochs > self.begin_epoch:  # [81,140]
            self.augmentor_replacement.__exit__()
