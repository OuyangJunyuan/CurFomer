...
void voxel_counting_kernel(const uint bid, const PointsInfo pf, const VoxInfo vf,
    ...
    slots[pid] = table.insert(key, [&](auto &val) {
...
__global__
void voxel_feats_scatter(const PointsInfo pf,
                         const float *__restrict__ points,
    ...
    auto *const feat = feats + (pf.num_feat + 1) * table.data[slot].val;
...
std::vector<at::Tensor> DynamicVoxelizationWrapper(const at::Tensor &points,
    ...
    auto slots = points.new_empty({num_point, 1}, torch::ScalarType::Int);
