...
__global__ void nms_kernel(const int boxes_num, const float nms_overlap_thresh,
    ...
    if (threadIdx.x < row_size) {
        ...
        int start = 0;
        if (row_start == col_start) {  // 如果是两组相同的boxes，则错位比较。
          start = threadIdx.x + 1;
        }
        for (i = start; i < col_size; i++) {  // col内那组里的boxes个数
...
// 把box分为每64个分为一组。然后共有boxes_num//64组，组和组之间进行iou计算并判定是否大于阈值，然后记录在mask里的一个bit内。
