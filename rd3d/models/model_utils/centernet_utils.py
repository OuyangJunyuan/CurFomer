...
def _topk_1d(scores, batch_size, batch_idx, obj, K=40, nuscenes=False):
    ...
    for bs_idx in range(batch_size):
        ...
        if obj.shape[-1] == 1 and not nuscenes:
            ...
            topk_score, topk_ind = torch.topk(obj[topk_inds.view(-1)].squeeze(-1),
                                              K)  # torch.topk(topk_scores.view(-1), K)
        else:
...
def decode_bbox_from_voxels_nuscenes(batch_size, indices, obj, rot_cos, rot_sin,
                                     center, center_z, dim, vel=None, iou=None, point_cloud_range=None, voxel_size=None,
                                     voxels_3d=None,
    ...
    for k in range(batch_size):
        ...
        cur_scores = final_scores[k, cur_mask]
        cur_labels = final_class_ids[k, cur_mask]
        cur_add_features = [add_feature[k, cur_mask] for add_feature in
                            add_features] if not add_features is None else None
