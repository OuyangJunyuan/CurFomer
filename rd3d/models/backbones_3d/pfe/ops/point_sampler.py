...
@SAMPLERS.register('select', 'index')
...
class RandomVoxelSampling(PS):
    ...
    def forward(self, xyz: torch.Tensor, **kwargs) -> torch.Tensor:
        if self.adaptive:
            self.gen: HierarchicalAdaptiveVoxelSampling = SAMPLERS.build_from_cfg(
...
@SAMPLERS.register('ctr_aware', 'ctr')
...
@SAMPLERS.register('hvcs_v2', 'hvcs', 'havs')
