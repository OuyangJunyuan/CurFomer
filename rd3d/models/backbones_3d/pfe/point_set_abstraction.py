...
class GeneralPointSetAbstraction(nn.Module):
    def __init__(self, ab_layer_cfg, input_channels: int):
        ...
        for sampler_cfg in self.cfg.get('samplers', []):
            self.samplers.append(SAMPLERS.build_from_cfg(sampler_cfg, parent=self, input_channels=self.input_channels))

        self.groupers = nn.ModuleList()
        for grouper_cfg in self.cfg.get('groupers', []):
            self.groupers.append(GROUPERS.build_from_cfg(grouper_cfg, parent=self, input_channels=self.input_channels))

        self.agg = AGGREGATIONS.build_from_cfg(
