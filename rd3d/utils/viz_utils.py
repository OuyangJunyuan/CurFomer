...
def add_points(vis, points, colors=None, offset=None, name=None, material=None):
    ...
    o3d_geometry.colors = open3d.utility.Vector3dVector(np.ascontiguousarray(colors))
    if material is None:
...
def add_keypoint(vis, points, radius=0.05, color=None, offset=None, name=None, material=None):
    for i in range(points.shape[0]):
        ...
        if material is None:
            ...
            vis.add_geometry("%s_%d" % (name, i), mesh_sphere, material)
...
def add_boxes(vis, boxes, labels=None, scores=None, color=None, offset=None, name=None, material=None):
    ...
    if color is not None and isinstance(color, (list, tuple)): color = np.array(color)

    for i in range(boxes.shape[0]):
        box_lines = translate_boxes_to_open3d_instance(boxes[i])
        if color is None:
            try:
                c = box_colormap[labels[i]]
            except:
                c = (1, 0, 0)
        else:
            c = color[i] if len(color.shape) == 2 else color
