...
class DataBaseSampler(object):
    def __init__(self, root_path, sampler_cfg, class_names, logger=None):
        ...
        self.disable_at_last_n_epoch = sampler_cfg.get('DISABLE_AT_LAST_N_EPOCH', 0)
    ...
    def collect_image_crops_kitti(self, info, data_dict, obj_points, sampled_gt_boxes, sampled_gt_boxes2d, idx):
        from ..datasets.kitti.kitti_object_eval_python import kitti_common
        from ..datasets.kitti import kitti_calibration
        calib_file = kitti_common.get_calib_path(int(info['image_idx']), self.root_path, relative_path=False)
        sampled_calib = kitti_calibration.Calibration(calib_file)
