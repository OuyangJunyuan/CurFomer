...
from . import kitti_utils, kitti_calibration, kitti_object3d
...
class KittiDataset(DatasetTemplate):
    ...
    def __init__(self, dataset_cfg, training=True, logger=None):
        ...
        with open(split_dir) as f:
            self.sample_id_list = [x.strip() for x in f.readlines()] if split_dir.exists() else None

        self.kitti_infos = []
        self.include_kitti_data(self.mode)
        if self.dataset_cfg.get('debug', False):
            self.kitti_infos = self.kitti_infos[:16]
    ...
    def get_label(self, idx):
        ...
        assert label_file.exists()
        return kitti_object3d.get_objects_from_label(label_file)
...
def create_kitti_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    dataset = KittiDataset(dataset_cfg=dataset_cfg, training=False)
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_kitti_infos':
        ...
        dataset_cfg = Config.fromfile(sys.argv[2]).DATASET
        create_kitti_infos(
            dataset_cfg=dataset_cfg,
