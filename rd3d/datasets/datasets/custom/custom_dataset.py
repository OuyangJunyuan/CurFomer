...
class CustomDataset(DatasetTemplate):
    default_class_names = ('Vehicle', 'Pedestrian', 'Cyclist')
    class_map = {
        'car': 'Vehicle',
        'truck': 'Vehicle',
    ...
    map_class_to_kitti = {'Vehicle': 'Car',
                          'Pedestrian': 'Pedestrian',
                          'Cyclist': 'Cyclist'}
...
def create_custom_infos(dataset_cfg, workers=4):
    ...
    dataset = CustomDataset(dataset_cfg, training=False)
    ...
    for sequence in tqdm(iterable=sequences_root.iterdir(), desc='sequences', leave=True):
        ...
        for sample_idx, (points_path, timestamp, pose) in enumerate(zip(points_paths, timestamps, poses)):
            pc_info = {
