...
def pcd2bin(file_path, save_dir):
    ...
    try:
        ...
        pts_ = o3d.io.read_point_cloud(str(file_path), remove_nan_points=True, remove_infinite_points=True)
        pts_ = np.asarray(pts_.points).copy()
        pts_ = np.insert(pts_, 3, 0, 1).astype(np.float32)
