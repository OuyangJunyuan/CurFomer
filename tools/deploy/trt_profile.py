...
def evaluate(engine_file, dataloader, use_build_in, num_points, nodes):
    import torch
    import numpy as np
    ...
    with engine.create_execution_context() as context:
        stream = cuda.Stream()
        print(engine.get_binding_shape(engine.get_binding_index("points"))[1])
        ...
        infer()  # warmup
        if use_build_in:
            context.profiler = MyProfiler(nodes)
        cuda.start_profiler()
        for i in range(100 if use_build_in else 2):
            infer()
        if use_build_in:
...
def main():
    ...
    parser.add_argument('--nodes', type=str, nargs='+', default=["HAVSampling", "BallQuery", "ForeignNode", "NMSBEV"])
    _, dataloader, cfgs, args = quick_demo(parser)

    use_build_in = args.build_in
    engine_file = args.engine

    evaluate(engine_file, dataloader, use_build_in, args.points, args.nodes)
