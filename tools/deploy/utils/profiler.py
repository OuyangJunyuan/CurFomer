...
class MyProfiler(trt.IProfiler):
    ...
    def print(self):
        ...
        table = prettytable.PrettyTable(
            ["name", "layers", "average (ms)", "a percentage (%)", "median (ms)", "m percentage (%)"])
        table.set_style(prettytable.MARKDOWN)
        for k, v in self.collect_infos.items():
            av = np.array(v).sum(axis=0)
            mean = np.mean(av)
            median = np.median(av)
            table.add_row((k, len(v), "%.3f" % mean, "%.1f" % (100 * mean / t_mean), "%.3f" % median,
                           "%.1f" % (100 * median / t_median)))
        table.align = "r"
        table.add_row(("Total", sum(len(v) for v in self.collect_infos.values()),
                       "%.1f" % t_mean, "%.1f" % 100, "%.1f" % t_median, "%.1f" % 100))
        print(table)
